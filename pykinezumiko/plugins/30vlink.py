    ...
    def on_command_debug_link(self, sender: int, user: int, amount: str):
        ...
        self.send(
            ...
            self.vlink_subscribe(
                user, f".debug link {time.asctime()}", round(dollars * 100), days, True
            ),
        )
        self.send(conf.INTERIOR, f"[{self.name(sender)}] 增加了 #{days} 日 #{user} 的订阅时长。")
    ...
    def vlink_subscribe(
        ...
        for _, subscription in Subscription.items():
            if subscription.user == user and subscription.identifier == identifier:
                self.send(conf.INTERIOR, f"{user} 试图多次发送具有相同账单时间 {identifier} 的图像。")
                return "该记录已确认过。"
        ...
        expiry = time.strftime("%-Y 年 %-m 月 %-d 日", time.gmtime(expiry + 3600 * 8))
