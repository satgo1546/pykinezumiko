...
def test_format_object():
    ...
    assert format_object(True) == "✓"
    assert format_object(False) == "✗"
    assert format_object(math.tau) == "6.283185307179586"
    assert format_object(19e19) == "1.9e20"
    assert format_object(-19e-19) == "-1.9e-18"
    ...
    assert format_object(1 + 2j) == "1+2i"
    assert format_object(3 - 4j) == "3-4i"
    assert format_object(-5 + 6j) == "-5+6i"
    assert format_object(complex(7)) == "7+0i"
    assert format_object(-8j) == "-8i"
    assert format_object(complex(math.nan, math.nan)) == "NaN+NaNi"
    assert format_object("'\f\0\0\b\a\r'") == "''␌␀␀␈␇␍''"
    assert format_object(b"\f\0\0\b\a\r") == "b'␌␀␀␈␇␍'"
    assert format_object(bytearray(b"foobar")) == "b[foobar]"
    ...
    assert format_object([114 + 514j, -1919 - 810j]) == "[114+514i -1919-810i]"
    assert format_object({}) == "{}"
    assert format_object({"a": 1}) == "{a: 1}"
    assert format_object({"a": None}) == "{a∅}"
    assert format_object({"x": 1, "y": 2}) == "{x: 1, y: 2}"
    assert format_object({114: "foo", 514: "bar"}) == "{114: 'foo', 514: 'bar'}"
    assert format_object(set()) == "{}"
    assert format_object({1, 2, 3}) == "{1, 2, 3}"
    assert format_object(frozenset((114, 514))) == "{114, 514}"
    assert format_object({("foo",): b"ar"}) == "{('foo'): b'ar'}"
    assert format_object(format_object) == repr(format_object)
