...
def send(context: int, text: str) -> None:
    """发送消息。

    :param context: 发送目标，正数表示好友，负数表示群。
    :param text: 要发送的消息内容，富文本用木鼠子码表示。
    """
    """转换木鼠子码字符串到消息段列表。"""
...
def _dispatch_message(event: Event) -> object:
...
def on_event(self, context: int, sender: int, data: dict[str, Any]) -> bool:
    ...
    match data:
        case {"post_type": "message", "message": message, "message_id": id}:
            ...
            message = _segments_to_str(message)
            result = self._dispatch_message(Event(context, sender, message, id))
        case {"request_type": ("friend"|"group") as request_type, "comment": message, "flag": flag}:
            # 这个类型的上报只有申请添加好友和申请加入群聊两种。
            result = self.on_admission(Event(context, sender, message, 0))
        ...
        case {"notice_type": "friend_recall" | "group_recall", "message_id": id}:
            # TODO
            message = self.onebot("get_msg", message_id=id)
            message = str(message["raw_message"]) if "raw_message" in message else ""
            result = self.on_message_deleted(context, sender, message, id)
        case {"notice_type": "offline_file", "file": {"name": name, "size": size, "url": url}}:
            result=self._dispatch_message(Event(context, sender, f"\a<File {url}#size={size}>{name}", 0))
...
@overload
def get_name(context: int, sender: int) -> str:
...
@overload
def get_name(context: tuple[int, int]) -> str: ...

@overload
def get_name(context: int) -> str:
    """获取好友名（正参数）或群聊名（负参数）。

    有Python侧一级缓存和go-cqhttp侧二级缓存，因此可以安心频繁调用本方法。
    """

def get_name(context, sender=None) -> str:
    if sender is not None:
        return get_name((context, sender))
    if context in _name_cache:
        return _name_cache[context]
    if isinstance(context, int):
        if context >= 0:
            for response in onebot("get_friend_list"):
                _name_cache[response["user_id"]] = response["nickname"]
            name = _name_cache.get(context, "")
        else:
            response = onebot("get_group_info", group_id=-context)
            name = response["group_name"]
    else:
        if context[0] >= 0:
            name = get_name(context[1])
        else:
            response = onebot("get_group_member_info", group_id=-context[0], user_id=context[1])
            name = response.get("card") or response["nickname"]
    _name_cache[context] = name
    return name
